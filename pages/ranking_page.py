"""Renderização da página de classificação por temporada."""
...
def render_season_page(config: SeasonConfig) -> None:
    MODO_COLUNAS_2026 = config.modo_colunas_2026
    CFG_PDF_2026 = {"formato_novo": config.formato_pdf_novo}
    ...
    # Abre o PDF
    with pdfplumber.open(config.tabela_pdf) as pdf:
        pagina = pdf.pages[0]
    ...
    if CFG_PDF_2026["formato_novo"]:
        df = normalizar_pdf_stockcar_2026(df)
    else:
    ...
    # Input do usuário para a última corrida (default: PDF + config da temporada)
    _default_corrida = max(
        detectar_ultima_corrida(df, fallback=config.default_ultima_corrida),
        config.default_ultima_corrida,
    )
    ultima_corrida = st.number_input(
        config.ultima_corrida_label,
        min_value=1,
        max_value=24,
        value=int(_default_corrida),
        step=1,
    )
    ...
    with tabs[7]:
        def carregar_dados_qualifying():
            """Carrega PDFs Q*.pdf da pasta de qualifying da temporada."""
            dados_qualifying = {}
            pasta_qualifying = config.qualifying_dir
            if not os.path.isdir(pasta_qualifying):
                st.warning(f"Pasta de qualifying não encontrada: {pasta_qualifying}")
    ...
    with tabs[8]:
        ...
        excecoes = config.qualifying_excecoes
        ...
        def _stats_piloto(piloto: str):
            if piloto not in _stats_cache:
                _stats_cache[piloto] = calcular_estatisticas(dados_qualifying, piloto)
            return _stats_cache[piloto]

        for piloto in pilotos_unicos:
            _media = _stats_piloto(piloto)[1]
            estatisticas_pilotos.append({
                "Piloto": piloto,
    ...
    with tabs[10]:
        ...
        with st.form(key='form_recorte'):
            intervalo = st.slider(
                "Selecione um intervalo de etapas (caso não queira usar o slider, deixe fixo em 0)",
                0, 24, (5, 15),
            )
            etapas_opcoes = [str(i) for i in range(1, ultima_corrida + 1)]
        ...
        if submit_button:
            if intervalo[1] > ultima_corrida:
                ...
                if intervalo[0] == 0 and intervalo[1] == 0:
                    ...
                    etapas_final = set(etapas_opcoes[intervalo[0] - 1 : intervalo[1]]) | set(
                        etapas_selecionadas
                    )
                ...
                df_recorte['Soma'] = df_recorte[
                    [etapa for etapa in etapas_final if etapa in df_cortez.columns]
                ].sum(axis=1)
